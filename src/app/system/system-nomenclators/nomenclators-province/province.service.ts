...
export class ProvinceService {
  private url = Config.urlApiRes() + '/nomenclator/province';

  private functionality = 'NPROVINCE';
  ...
  new(province: Province): Observable<any> {

    const options = {
      headers: Config.getheaders(this.functionality)
    };

    return this.http.post(`${this.url}`, province, options)
      .pipe( map(r => r),
        catchError(this.handleError<any>('insert')))
      ;
  }

  update(province: Province, id: string): Observable<any> {
    let params = new HttpParams();
    params = params.append('id', id);
    ...
    return this.http.put(`${this.url}`, province, options)
      .pipe( map(r => r),

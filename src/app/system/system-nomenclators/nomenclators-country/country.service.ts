...
export class CountryService {
  private url = Config.urlApiRes() + '/nomenclator/country';

  private functionality = 'NCOUNTRY';
  ...
  new(country: Country): Observable<any> {

    const options = {
      headers: Config.getheaders(this.functionality)
    };

    return this.http.post(`${this.url}`, country, options)
      .pipe( map(r => r),
        catchError(this.handleError<any>('insert')))
      ;
  }

  update(country: Country, id: string): Observable<any> {
    let params = new HttpParams();
    params = params.append('id', id);
    ...
    return this.http.put(`${this.url}`, country, options)
      .pipe( map(r => r),

...
export class ActivityService {private url = Config.urlApiRes() + '/nomenclator/activity';

  private functionality = 'NACTIVITY';
  ...
  new(activity: Activity): Observable<any> {

    const options = {
      headers: Config.getheaders(this.functionality)
    };

    return this.http.post(`${this.url}`, activity, options)
      .pipe( map(r => r),
  ...
  update(activity: Activity, id: string): Observable<any> {
    ...
    return this.http.put(`${this.url}`, activity, options)
      .pipe( map(r => r),
